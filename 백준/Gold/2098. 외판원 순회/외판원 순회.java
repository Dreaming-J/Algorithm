...
public class Main {
    ...
    static final int INF = 16_000_000, START = 0;
    static int citySize;
    static int[][] cities;
    static int[][] dpCost;
    ...
    public static int findRoute(int cityIdx, int visitedBit) {
    	//2-1. 모든 도시 방문했다면, 원점으로 돌아오는 비용 반환
    	if (visitedBit == (1 << citySize) - 1)
    		return cities[cityIdx][START] == 0 ? INF : cities[cityIdx][START];
    	
    	//2-2. 방문하지 않은 도시의 최적 경로를 이미 알고 있다면, 해당 값 반환
    	if (dpCost[cityIdx][visitedBit] != -1)
    		return dpCost[cityIdx][visitedBit];
    	
    	//2-3. 현재 상태에서 남은 도시까지의 모든 경로 최적화 후, 찾은 값 반환
    	dpCost[cityIdx][visitedBit] = INF;
    	for (int nextIdx = 1; nextIdx < citySize; nextIdx++) {
    		//이미 방문했다면 패스
    ...
    public static void main(String[] args) throws IOException {
        ...
        //2. 최소 비용 탐색
        //3. 출력
        System.out.println(findRoute(START, 0b1));
    }
    ...
    public static void init() throws IOException {
    	...
    	//1-2. 도시의 이동 비용을  인접 행렬 방식으로 입력
    	cities = new int[citySize][citySize];
    	for (int from = 0; from < citySize; from++) {
    		st = new StringTokenizer(input.readLine());
        	for (int to = 0; to < citySize; to++) {
        		cities[from][to] = Integer.parseInt(st.nextToken());
        	}
    	}
    	
    	//1-3. 변수 초기화
    	dpCost = new int[citySize][1 << citySize];
    	for (int idx = 0; idx < citySize; idx++)
    		Arrays.fill(dpCost[idx], -1);
    }
}
